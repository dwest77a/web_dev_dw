var rsg_colour = {
	...
	changeColoursOfHighlightedLayer: function()
	{
		...
		canvas.width =colourbar_image.naturalWidth;
		canvas.height =colourbar_image.naturalHeight;
		var ctx = canvas.getContext("2d");
		ctx.drawImage(colourbar_image, 0, 0);
		var data = ctx.getImageData(0, 0, canvas.width, canvas.height).data;
		
		var colourbar_list_colours_temp = rsg_colour.getColoursInImage(colourbar_image)
	...
	populateColourSelection: function(force)
	{ 
		if (force == undefined){
			...
			{
				var colourlabel1 = all_colour_cells[iterator].innerHTML.toUpperCase()
				var colourlabel2 = all_colour_choices[iterator+1].value
				if((rsg_colour.colourChanged(colourlabel1, colourlabel2))
			...
			rsg_ui.close(["colour-cell-box"]);
			rsg_colour.changeColoursOfHighlightedLayer();
			var temp1 = rsghelper_layers.get_layer_copy_by_id(0).assigned_subproperties;
		}
		else {
			var colourbar_url = get_colourbar_url(rsghelper_layers.get_layer_copy_by_id(highlighted_layer_id).variable_index, 
	                                  rsghelper_layers.get_layer_copy_by_id(highlighted_layer_id).project_table);
			var original_img = new Image;
			original_img.src = colourbar_url;
			original_colours_list = rsg_colour.getColoursInImage(original_img);
			for(iterator=0; iterator < original_colours_list.length; iterator++){
				// Force reinitialisation of layer colours
			}
		}
	},
	
	toggle_colour_cell_box: function()
	{
		// since the colourbar is displayed for an active (highlighted) layer, ...
		// ... get its id for reference:
		highlighted_layer_id = rsghelper_layers.getHighlightedLayerId();
		
		rsg_ui.open(["colour-cell-box"]);
		...
		var old_image = document.getElementById("colourbar_image")
		var old_colourbar_colour_list = rsg_colour.getColoursInImage(old_image)
		
		var colourbar_url = get_colourbar_url(rsghelper_layers.get_layer_copy_by_id(highlighted_layer_id).variable_index, 
	                                  rsghelper_layers.get_layer_copy_by_id(highlighted_layer_id).project_table);
		var original_img = new Image;
		original_img.src = colourbar_url; // Reset to default every time colour-cell-box is opened
		// pull the entire image into an array of pixel data
		
		// get all available colours to be changed:get
		colourbar_colour_list = rsg_colour.getColoursInImage(original_img);
		old_colour_list = document.getElementById("colour-list-container");
		old_colour_list.remove();
		if (old_colourbar_colour_list.length != colourbar_colour_list.length){
			rsg_colour.populateColourSelection(true);
		}
		
		colour_list_obj = document.createElement('div');
		colour_list_obj.setAttribute('id', 'colour-list-container');
		document.getElementById("colour-cell-box").appendChild(colour_list_obj);
		...
		for (iterator = 0; iterator < colourbar_colour_list.length; iterator++)
		{
			if((colourbar_colour_list[iterator] != "#000000") && (colourbar_colour_list[iterator] != "#ffffff"))
			{
				...
				colour_cell.textContent = old_colourbar_colour_list[iterator].replace('#','');
				colour_cell.setAttribute('class','colour-cell') ;
				colour_cell.setAttribute('style','display: none;');
				...
				blank_space.setAttribute('class','blank-space');
				blank_space.setAttribute('id', 'blank space'+iterator);
				// checkbox.setAttribute("style", "background-color: " + colourbar_colour_list[iterator] + ";" + "color: " + colourbar_colour_list[iterator] + ";");
        ...
        transparent_checkbox.name = "todo[]";
				...
				colour_list_obj.appendChild(blank_space);
	...
	getColoursInImage: function(img)
	{
		...
		for(var i = 0, n = data.length; i < n; i += 4)
		{
			...
			if (($.inArray(r,red_in_image) == -1) || ($.inArray(g,green_in_image) == -1) || ($.inArray(b,blue_in_image) == -1)){
				var hex = rsg_colour.rgb2hex("rgb("+r+","+g+","+b+")");
				colours_in_image.push(hex);
						
				red_in_image.push(r);
				green_in_image.push(g);
				blue_in_image.push(b);
			}
		}
