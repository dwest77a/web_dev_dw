...
function generate_png_url(variable_index, project_index, proj_table, layer_id, average) 
{
	...
	if (base_url_arr[base_url_arr.length-2] == "l3_monthly"){
		var ymd_arr = ymd.split("/");
		var ymd = ymd_arr[0] + "/" + ymd_arr[1];
	} 
...
function load_logo_config_table() {
	...
	$.ajax({
		type: "GET",
		url: "../rsg_data_viewer_config/logo_lookup.txt",
		dataType: "text",
		async: false,
		success: function (data) {
			data = removeComments(data);
...
function get_header_by_project_index(project_index) {
	...
	catch(e)
	{
		...
		alert("Sorry. Image loading not caught up. You need to reload page (try using the 'Get shared URL' button to maintain current view and data selection).")
	}
